...
testOneProgram() {
    ...
    cp /root/sseq/compile_commands/example1/compile_commands.json "$programDir/compile_commands.json"

    #echo "running clang-tidy"
    #clang-tidy $programName -checks=-*,readability-braces-around-statements -fix-errors
    
    # echo "running timeout 20 $SSEQ $programName div %any% mut 2>1"
    # res=`timeout 20 $SSEQ $programName div %any% mut 2>1`
    # echo "running timeout 20 $SSEQ $programName null 2>1"

    #res=`$SSEQ $programName null 2>1`

    #res=`$SSEQ $programName null >$programName.txt`
    #res=`$SSEQ $programName div %any% mut >$programName.txt`
    #res=`$SSEQ $programName shf %any% mut >$programName.txt`
    res=`$SSEQ $programName shf %any% mut 2>1`
    #res=`$SSEQ $programName null 2>1`
...
testing() {
    ...
    do
        ...
        if [[ $filename == *_null.c || $filename == *_null.cpp ]]; then
            ((number++))
            continue
        fi
...
init() {
    ...
    do
        ...
        if [[ $filename == *_shf.c || $filename == *_shf.cpp ]]; then
            rm "$filename"
        fi
    done <<< "$res"
}
...
replace() {
    ...
    while read -r filename; do
        ...
        if [[ $filename == *_null.c ]]; then
            target="${filename/_null.c/.c}"
            cat "$filename" > "$target"
            rm "$filename"
        elif [[ $filename == *_null.cpp ]]; then
            target="${filename/_null.cpp/.cpp}"
            cat "$filename" > "$target"
            rm "$filename"
        fi

        if [[ $filename == *_shf.c ]]; then
            target="${filename/_shf.c/.c}"
            cat "$filename" > "$target"
            rm "$filename"
        elif [[ $filename == *_shf.cpp ]]; then
            target="${filename/_shf.cpp/.cpp}"
            cat "$filename" > "$target"
            rm "$filename"
        fi
    done <<< "$res"
...
if [ "${1}" != "--source-only"  ]; then
    ...
    # replace2
    # testOneProgram /home/sd/testcode/test_org.cpp
    # testOneProgram /home/sd/SanitizerDector/juliet_dataset/C/testcases/CWE457_Use_of_Uninitialized_Variable/s01/CWE457_Use_of_Uninitialized_Variable__char_pointer_01.c
