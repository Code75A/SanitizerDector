...
public:
  explicit SeqASTVisitor(clang::ASTContext *ctx, clang::Rewriter &R,
                         SeqInfo &seq_i)
  ...
  bool getFlag(int index) {
    if (index >= FLAG_WIDTH)
      ...
      return seq_info.vflags[index];
  };

  // TODO: 将函数参数全部转为全局从而重构
  void judgeShf(const clang::BinaryOperator *bop,
                const clang::BinaryOperator *last_bop, std::string *insertStr,
                clang::SourceManager &SM, int &bits);
  void judgePrint(const clang::BinaryOperator *bop, clang::SourceManager &SM,
                  const int c, clang::SourceLocation &loc);
  void judgeDiv(const clang::BinaryOperator *bop, std::string *insertStr,
                int &count, clang::SourceManager &SM, const int c,
                clang::SourceLocation &loc);

  void judgeDivWithoutUBFuzz(const clang::BinaryOperator *bop,
  ...
  void judgeShfWithoutUBFuzz(const clang::BinaryOperator *bop,
                             std::string *insertStr, int &count,
                             clang::SourceManager &SM,
                             clang::Rewriter &_rewriter,
  ...
  void JudgeAndInsert(clang::Stmt *&stmt, const clang::BinaryOperator *bop,
                      clang::Rewriter &_rewriter, int &count,
                      clang::SourceManager &SM, std::string type,
                      std::string mode, clang::SourceLocation &DefHead);

  void JudgeAndPtrTrack(clang::Stmt *stmt, int cur_count,
                        clang::SourceManager &SM,
                        clang::SourceLocation &DefHead,
                        clang::SourceLocation &insertLoc);

  void JudgeByMode(clang::Stmt *&stmt, const clang::BinaryOperator *bop,
                   clang::Rewriter &_rewriter, int &count,
                   clang::SourceManager &SM, std::string type,
                   std::string stmt_string, std::string mode,
                   clang::SourceLocation &DefHead);

  void LoopChildren(clang::Stmt *&stmt, const clang::BinaryOperator *bop,
                    clang::Rewriter &_rewriter, int &count,
                    clang::SourceManager &SM, std::string type,
                    std::string stmt_string, std::string mode,
                    clang::SourceLocation &DefHead);
...
public:
  explicit SeqASTConsumer(clang::CompilerInstance &CI, clang::Rewriter &R,
                          SeqInfo &seq_i)
      : _visitor(&CI.getASTContext(), R, seq_i) {}

  virtual void HandleTranslationUnit(clang::ASTContext &ctx) {
    _visitor.TraverseDecl(
        ctx.getTranslationUnitDecl()); // 这句话开始触发Visit系列函数
  }
...
public:
  ...
  CreateASTConsumer(clang::CompilerInstance &compiler,
    ...
    return std::make_unique<SeqASTConsumer>(compiler, _rewriter, seq_info);
  }
  // 写文件
