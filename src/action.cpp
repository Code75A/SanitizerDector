...
marco function now can be correctly instrumented

*/

/* update2
introduce codeFormatter.sh to format generated .c files
only may expires at marco function without ending with ';'
these case need to be manually fixed
*/

/* update3
when process .cpp file
some func(&a , &b)
initial need to be process specially (false positive)
*/
#include "action.h"
namespace sseq {
// -----------null模式全局变量-----------
...
bool NeedLoopPtrTrack(std::string type) {
  if (type == "BinaryOperator" || type == "CallExpr" ||
      type == "UnaryOperator" || type == "ConditionalOperator")
    return true;
  else
    return false;
}
...
bool NotGlobalNorEnum(clang::ValueDecl *vdecl) {
  ...
  if (isa<clang::EnumConstantDecl>(vdecl))
    return false;
...
bool SeqASTVisitor::isInSystemHeader(const clang::SourceLocation &loc) {
  ...
  if (SM.getFilename(loc) == "")
    return true;
  std::string filename = SM.getFilename(loc).str();
  if (filename.find("/usr/include/") != std::string::npos)
    return true;
  if (filename.find("llvm-12.0.0.obj/") != std::string::npos)
    return true; // 结合实际情况 当前工具开发使用llvm12
  return false;
}
...
void GetSubExpr(clang::Stmt *&stmt, clang::Stmt *&ori_stmt, std::string type,
                bool &fir, clang::SourceManager &SM, int &UBFUZZ_line) {
...
void GetSimplifiedExpr(clang::Expr *&hs, const clang::BinaryOperator *&bop,
                       std::string type) {
  ...
  if (type == "ArraySubscriptExpr") {
    clang::ArraySubscriptExpr *phs =
        llvm::dyn_cast<clang::ArraySubscriptExpr>(hs);
...
void generateNullPtrAssign(std::string v_name, int cur_count,
                           std::string typestr, std::string *insertStr,
                           clang::SourceManager &SM, bool isGlobal) {
  ...
  *insertStr += v_name + "_copy_" + typestr + " = &" + file_name + "_" +
                global_current_func_name + "_Catcher_" +
                std::to_string(cur_count) + ";\n";

  return;
}
// mode[null]: 生成 Catcher = *_copy_ 的检测语句
void generateNullPtr(std::string v_name, int cur_count, std::string typestr,
                     std::string *insertStr, clang::SourceManager &SM,
                     bool isGlobal) {
  std::string file_name =
      SM.getFileEntryForID(SM.getMainFileID())->getName().str();
  SimplifiedFileName(file_name);
  ...
  typestr = strTrans(typestr);
  *insertStr = "" + file_name + "_" + global_current_func_name + "_Catcher_" +
               std::to_string(count) + "= *" + file_name + "_";
...
void generateArray(int n, std::string v_name, std::string *insertStr,
                   const std::string mode, clang::SourceManager &SM) {
  ...
  if (mode == "div")
    if (FOR_COND)
      *insertStr =
          ("((" + file_name + "_div_SaniCatcher" + num + " = " + file_name +
           "_div_SaniTestArr" + num + "[(" + v_name + " != 0) - 1]),");
    else
      *insertStr =
          ("(" + file_name + "_div_SaniCatcher" + num + " = " + file_name +
           "_div_SaniTestArr" + num + "[(" + v_name + " != 0) - 1]),");
    ...
    if (DO_WHILE_MACRO)
      *insertStr =
          ("(" + file_name + "_shf_SaniCatcher" + num + " = " + file_name +
           "_shf_SaniTestArr" + num + "[" + v_name + "]);");
    else
      *insertStr =
          ("(" + file_name + "_shf_SaniCatcher" + num + " = " + file_name +
           "_shf_SaniTestArr" + num + "[" + v_name + "]),");
  else
    ; // std::cout<<"Error: generateArray encounter invalid mode
      // type."<<std::endl;
  return;
}
// mode[null]: 生成 _copy_ = NULL; 的定义语句(需要stdddef.h库)
void initNullPtr(std::string v_name, std::string typestr,
                 std::string *insertStr, clang::SourceManager &SM,
                 bool isGlobal) {
  std::string file_name =
      SM.getFileEntryForID(SM.getMainFileID())->getName().str();
...
void initNullPtrCatcher(std::string v_name, int cur_count,
                        std::string *insertStr, clang::SourceManager &SM) {
  std::string file_name =
      SM.getFileEntryForID(SM.getMainFileID())->getName().str();
  SimplifiedFileName(file_name);

  *insertStr = "int " + file_name + "_" + global_current_func_name +
               "_Catcher_" + std::to_string(cur_count) + "=" +
               std::to_string(cur_count) + ";\n";
...
std::string initSani(int lst_n, int n, clang::SourceManager &SM) {
  ...
  std::string Arr =
      "int " + file_name + "_div_SaniTestArr" + std::to_string(lst_n) + "[10]";
  ...
  std::string Cat =
      "int " + file_name + "_div_SaniCatcher" + std::to_string(lst_n);
  for (int i = lst_n + 1; i < n; i++) {
...
std::string initSani_shf(int cur_count, int bits, clang::SourceManager &SM) {
  ...
  std::string Arr = "int " + file_name + "_shf_SaniTestArr" +
                    std::to_string(cur_count) + "[" + num + "];\n";

  std::string Cat = "int " + file_name + "_shf_SaniCatcher" +
                    std::to_string(cur_count) + ";\n";
...
void SeqASTVisitor::judgeShf(const clang::BinaryOperator *bop,
                             const clang::BinaryOperator *last_bop,
                             std::string *insertStr, clang::SourceManager &SM,
                             int &bits) {
  if (bop == nullptr) {
  ...
  if (Tool::get_stmt_string(lhs).find("+ (MUT_VAR)") != -1) {
    ...
    const clang::BinaryOperator *bopl =
    ...
    while (!bopl) {
      ...
      if (type == "ParenExpr") {
        ...
        clang::ImplicitCastExpr *plhs =
            llvm::dyn_cast<clang::ImplicitCastExpr>(lhs);
        lhs = plhs->getSubExpr();
        ...
        clang::CStyleCastExpr *plhs =
            llvm::dyn_cast<clang::CStyleCastExpr>(lhs);
        lhs = plhs->getSubExpr();
    ...
    const clang::BinaryOperator *bopr =
    ...
    while (!bopr) {
      ...
      if (type == "ParenExpr") {
        ...
        clang::ImplicitCastExpr *prhs =
            llvm::dyn_cast<clang::ImplicitCastExpr>(rhs);
        rhs = prhs->getSubExpr();
      } else if (type == "UnaryOperator") {
        clang::UnaryOperator *prhs = llvm::dyn_cast<clang::UnaryOperator>(rhs);
        rhs = prhs->getSubExpr();
      } else if (type == "CStyleCastExpr") {
        clang::CStyleCastExpr *prhs =
            llvm::dyn_cast<clang::CStyleCastExpr>(rhs);
        rhs = prhs->getSubExpr();
      } else if (type == "CallExpr") {
    ...
    clang::Expr *fin_lhs = last_bop->getLHS();
    clang::Expr *fin_rhs = last_bop->getRHS();
    ...
    if (type == "ImplicitCastExpr" || type == "CStyleCastExpr") {
      ...
      ; // std::cout<<"type:"<<type<<std::endl;
      bits = -1;
      ; // std::cout<<"Debug: Unexpected situation when generate
        // shf_Array."<<std::endl;
    }
...
void SeqASTVisitor::judgePrint(const clang::BinaryOperator *bop,
                               clang::SourceManager &SM, const int c,
  ...
  if (op == clang::BinaryOperator::Opcode::BO_LAnd &&
      SM.getSpellingColumnNumber(rhs->getBeginLoc()) < c &&
      SM.getSpellingColumnNumber(rhs->getEndLoc()) >= c) {
  ...
  if ((PosDivideZero(Tool::get_stmt_string(lhs))) &&
      SM.getSpellingColumnNumber(lhs->getEndLoc()) >= c) {
    ; // std::cout<<"check lhs:"<<Tool::get_stmt_string(lhs)<<std::endl;
...
void SeqASTVisitor::judgeDiv(const clang::BinaryOperator *bop,
                             std::string *insertStr, int &count,
                             clang::SourceManager &SM, const int c,
                             clang::SourceLocation &loc) {
  if (bop == nullptr) {
    ; // std::cout<<"warnning:encounter nullptr,maybe not a binaryoperator\n";
  ...
  if ((op == clang::BinaryOperator::Opcode::BO_Div ||
       op == clang::BinaryOperator::Opcode::BO_Rem) &&
      (SM.getSpellingColumnNumber(rhs->getEndLoc()) > c &&
       SM.getSpellingColumnNumber(rhs->getBeginLoc()) < c)) {
...
void SeqASTVisitor::judgeDivWithoutUBFuzz(const clang::BinaryOperator *bop,
                                          std::string *insertStr, int &count,
  ...
  if (op == clang::BinaryOperator::Opcode::BO_Div ||
    ...
    if (clang::IntegerLiteral *in =
            llvm::dyn_cast<clang::IntegerLiteral>(rhs)) {
      if (in->getValue().getZExtValue() == 0) {
        generateArray(count++, Tool::get_stmt_string(rhs), insertStr, "div",
                      SM);
        if (FOR_COND) {
          _rewriter.InsertText(SM.getSpellingLoc(SR.getBegin()), "(", true,
                               true);
        ...
        auto afterEnd =
            clang::Lexer::getLocForEndOfToken(SR.getEnd(), 0, SM, LO);
...
// mut插入模式下（无UBFUZZ），对某CallExpr
// bop下的AST中的所有可能的/与%进行查询和插入,位置固定在在CallExprHead防止误判为参数
void SeqASTVisitor::judgeDivWithoutUBFuzz(const clang::BinaryOperator *bop,
                                          std::string *insertStr, int &count,
                                          clang::SourceManager &SM,
                                          clang::Rewriter &_rewriter,
                                          clang::SourceLocation &CallExprHead) {
  ...
  if (PosDivideZero(Tool::get_stmt_string(lhs))) {
    const clang::BinaryOperator *bopl =
        llvm::dyn_cast<clang::BinaryOperator>(lhs); // TODO: IgnoreParenCAsts
    std::string type;
...
// mut插入模式下（无UBFUZZ），对bop下的AST中的所有可能的<<与>>进行查询和插入
void SeqASTVisitor::judgeShfWithoutUBFuzz(const clang::BinaryOperator *bop,
                                          std::string *insertStr, int &count,
                                          clang::SourceManager &SM,
                                          clang::Rewriter &_rewriter,
                                          clang::SourceLocation &DefHead) {
  if (bop == nullptr) {
    ; // std::cout<<"warnning:encounter nullptr,maybe not a binaryoperator\n";
    return;
  }
  ...
  if (op == clang::BinaryOperator::Opcode::BO_Shl ||
      op == clang::BinaryOperator::Opcode::BO_Shr ||
      op == clang::BinaryOperator::Opcode::BO_ShlAssign ||
      op == clang::BinaryOperator::Opcode::BO_ShrAssign) {

    if (!rhs->getBeginLoc().isMacroID()) {
      generateArray(count, Tool::get_stmt_string(rhs), insertStr, "shf", SM);

      // DeclStmt: After
      _rewriter.InsertTextAfter(SM.getExpansionLoc(lhs->getBeginLoc()),
                                *insertStr);
  ...
  if (PosShfOverflow(Tool::get_stmt_string(lhs))) {
    ...
    const clang::BinaryOperator *bopl =
        llvm::dyn_cast<clang::BinaryOperator>(lhs->IgnoreCasts());
    std::string type;
  ...
  if (PosShfOverflow(Tool::get_stmt_string(rhs))) {
    ...
    const clang::BinaryOperator *bopr =
        llvm::dyn_cast<clang::BinaryOperator>(rhs->IgnoreParenCasts());
    std::string type;
...
// TODO:注释
void SeqASTVisitor::JudgeAndInsert(clang::Stmt *&stmt,
                                   const clang::BinaryOperator *bop,
                                   clang::Rewriter &_rewriter, int &count,
                                   clang::SourceManager &SM, std::string type,
                                   std::string mode,
                                   clang::SourceLocation &DefHead) {
  ...
  if (type == "DeclStmt") {
    ...
    for (auto phs = declstmt->decl_begin(); phs != declstmt->decl_end();
         ++phs) {
      if (clang::VarDecl *varDecl = llvm::dyn_cast<clang::VarDecl>(*phs)) {
        if (varDecl->hasInit()) {
          ...
          if (llvm::isa<clang::BinaryOperator>(expr)) {
            ...
            // SM.getExpansionLoc(expr->getEndLoc());
            ...
            *insertStr = "";
            ...
            if (mode == "div")
              ...
              judgeShfWithoutUBFuzz(bop, insertStr, count, SM, _rewriter,
                                    DefHead);
            else
    ...
    for (int i = 0; i < numArgs; i++) {
      ...
      *insertStr = "";
      clang::SourceLocation insertLoc =
          SM.getExpansionLoc(callexpr->getBeginLoc());
...
// TODO
void SeqASTVisitor::JudgeAndPtrTrack(clang::Stmt *stmt, int cur_count,
                                     clang::SourceManager &SM,
                                     clang::SourceLocation &DefHead,
                                     clang::SourceLocation &insertLoc) {
  ...
  if (clang::DeclStmt *declstmt = llvm::dyn_cast<clang::DeclStmt>(stmt)) {
    for (auto it = declstmt->decl_begin(); it != declstmt->decl_end(); ++it) {
      if (auto var = llvm::dyn_cast<clang::VarDecl>(*it)) {
        ; // std::cout<<Tool::get_stmt_string(stmt)<<std::endl;
        if (var->hasInit() || var->getType()->isPointerType() ||
          ...
          *insertStr = "";
          ...
          // if(!isIntegerVar(var)){
          //    ;//std::cout<<"[SeqASTVisitor] Not an integer variable:
          //    "<<varName<<std::endl;
          //     delete insertStr;
          //     continue;
          // }
  ...
  else if (clang::BinaryOperator *bop =
    ...
    if (bop->isAssignmentOp()) {
      ...
      if (!isMallocAssignment(bop)) {
        ...
        if (rhs) {
          ...
          if (llvm::isa<clang::MemberExpr>(rhs) ||
              llvm::isa<clang::ArraySubscriptExpr>(rhs) ||
              llvm::isa<clang::DeclRefExpr>(rhs)) {
            if (SimplifyDeclRef(rhs, declexpr)) {
              if (!llvm::isa<clang::FunctionDecl>(declexpr->getDecl())) {
                ...
                if (NotInWNameList(varName) && NotGlobalNorEnum(value_decl) &&
                    isIntegerVar(value_decl)) {
          ...
          if (SimplifyDeclRef(lhs, declexpr)) {
            ...
            if (NotInWNameList(varName) && NotGlobalNorEnum(value_decl) &&
              ...
              *insertStr = "";
              ...
              if (value_decl->getDeclContext()->isTranslationUnit())
                generateNullPtrAssign(varName, cur_count, typestr, insertStr,
                                      SM, true);
              else
                generateNullPtrAssign(varName, cur_count, typestr, insertStr,
                                      SM, false);
      ...
      clang::Expr *lhs = bop->getLHS();
      if (lhs) {
        if (clang::BinaryOperator *lhs_bop =
                llvm::dyn_cast<clang::BinaryOperator>(lhs)) {
          clang::Stmt *lhs_stmt = IgnoreParenCastsStmt(lhs);
          JudgeAndPtrTrack(lhs_stmt, cur_count, SM, DefHead, insertLoc);
        }
      ...
      clang::Expr *rhs = bop->getRHS();
      if (rhs) {
        if (clang::BinaryOperator *rhs_bop =
                llvm::dyn_cast<clang::BinaryOperator>(rhs)) {
          clang::Stmt *rhs_stmt = IgnoreParenCastsStmt(rhs);
          JudgeAndPtrTrack(rhs_stmt, cur_count, SM, DefHead, insertLoc);
        }
  ...
  // a++;
  else if (clang::UnaryOperator *uexpr =
  ...
  // print(a);
      ...
      if (SimplifyDeclRef(arg, declexpr)) {
        ...
        if (NotInWNameList(varName) && NotGlobalNorEnum(value_decl) &&
          ...
          *insertStr = "";
          ...
          // clang::SourceLocation end =
          // clang::Lexer::getLocForEndOfToken(callexpr->getEndLoc(), 0, SM,
          // LangOpts);
          _rewriter.InsertTextAfter(insertLoc, *insertStr);
  ...
  } else if (clang::ConditionalOperator *cond =
    ...
    if (falseExpr) {
      ...
      if (SimplifyDeclRef(falseExpr, declexpr)) {
        ...
        if (NotInWNameList(varName) && NotGlobalNorEnum(value_decl) &&
            isIntegerVar(value_decl) &&
            !value_decl->getType()->isReferenceType()) {
          std::string *insertStr = new std::string;
          *insertStr = "";
...
void SeqASTVisitor::JudgeByMode(clang::Stmt *&stmt,
                                const clang::BinaryOperator *bop,
                                clang::Rewriter &_rewriter, int &count,
                                clang::SourceManager &SM, std::string type,
                                std::string stmt_string, std::string mode,
                                clang::SourceLocation &DefHead) {
  if (NeedLoopChildren(type))
    LoopChildren(stmt, bop, _rewriter, count, SM, type, stmt_string, mode,
                 DefHead);
  else if (mode == "div") {
...
void SeqASTVisitor::LoopChildren(clang::Stmt *&stmt,
                                 const clang::BinaryOperator *bop,
                                 clang::Rewriter &_rewriter, int &count,
                                 clang::SourceManager &SM, std::string type,
                                 std::string stmt_string, std::string mode,
  ...
  if (type == "ForStmt") {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      JudgeByMode(s_stmt, bop, _rewriter, count, SM, type, stmt_string, mode,
                  DefHead);
    }
    FOR_COND = false;
    ...
    if (clang::CompoundStmt *CS = llvm::dyn_cast<clang::CompoundStmt>(sbody))
      for (clang::Stmt *s_stmt : CS->body()) {
        ...
        // if(mode == "null"){
        //     const clang::LangOptions& LangOpts = _ctx->getLangOpts();
        //     clang::SourceLocation afterLBrace =
        //     clang::Lexer::getLocForEndOfToken(
      ...
      if (!llvm::isa<clang::IfStmt>(sbody) &&
          !llvm::isa<clang::ForStmt>(sbody) &&
          !llvm::isa<clang::NullStmt>(sbody)) {
        // If sbody is a simple expression/return/decl statement, skip wrapping
        if (llvm::isa<clang::Expr>(sbody) ||
            llvm::isa<clang::ReturnStmt>(sbody) ||
            llvm::isa<clang::DeclStmt>(sbody) ||
            llvm::isa<clang::GotoStmt>(sbody)) {
          ; // leave as-is
        } else {
          // Atomically replace the statement token-range with a braced block.
          clang::SourceRange SR = get_sourcerange(sbody);
          auto charRange = clang::CharSourceRange::getTokenRange(SR);
          clang::LangOptions LangOpts = _rewriter.getLangOpts();
          llvm::StringRef origRef =
              clang::Lexer::getSourceText(charRange, SM, LangOpts);
          std::string origText = origRef.str();
          size_t pos = origText.find_first_not_of(" \t\n\r");
          if (pos == std::string::npos || origText[pos] != '{') {
            std::string repl = std::string("{") + origText + std::string("}");
            _rewriter.ReplaceText(SR, repl);
          }
        }
      }
      ...
      if (NeedLoopChildren(type))
        LoopChildren(s_stmt, bop, _rewriter, count, SM,
                     s_stmt->getStmtClassName(), stmt_string, mode, DefHead);

      else if (mode == "div") {
        if (PosDivideZero(stmt_string))
          JudgeAndInsert(sbody, bop, _rewriter, count, SM,
                         sbody->getStmtClassName(), mode, DefHead);
      } else if (mode == "shf") {
        if (PosShfOverflow(stmt_string))
          JudgeAndInsert(sbody, bop, _rewriter, count, SM,
                         sbody->getStmtClassName(), mode, DefHead);
    ...
    if (scond) {
      ...
      if (NeedLoopChildren(type))
        LoopChildren(scond, bop, _rewriter, count, SM,
                     scond->getStmtClassName(), stmt_string, mode, DefHead);

      else if (mode == "div") {
        if (PosDivideZero(stmt_string))
          JudgeAndInsert(scond, bop, _rewriter, count, SM,
                         scond->getStmtClassName(), mode, DefHead);
      } else if (mode == "shf") {
        if (PosShfOverflow(stmt_string))
          JudgeAndInsert(scond, bop, _rewriter, count, SM,
                         scond->getStmtClassName(), mode, DefHead);
    ...
    type = sbody->getStmtClassName();
    if (clang::CompoundStmt *while_comp =
            llvm::dyn_cast<clang::CompoundStmt>(sbody))
      for (auto &s_stmt : while_comp->children()) {
        ...
        if (NeedLoopChildren(type))
          ...
          if (PosDivideZero(stmt_string))
            JudgeAndInsert(s_stmt, bop, _rewriter, count, SM,
                           s_stmt->getStmtClassName(), mode, DefHead);
        } else if (mode == "shf") {
          if (PosShfOverflow(stmt_string))
            JudgeAndInsert(s_stmt, bop, _rewriter, count, SM,
                           s_stmt->getStmtClassName(), mode, DefHead);
      ...
      LoopChildren(sbody, bop, _rewriter, count, SM, type, stmt_string, mode,
                   DefHead);
    }
  } else if (type == "DoStmt") {
    ...
    if (NeedLoopChildren(type))
      LoopChildren(icond, bop, _rewriter, count, SM, icond->getStmtClassName(),
                   stmt_string, mode, DefHead);

    else if (mode == "div") {
      if (PosDivideZero(stmt_string))
        JudgeAndInsert(icond, bop, _rewriter, count, SM,
                       icond->getStmtClassName(), mode, DefHead);
    } else if (mode == "shf") {
      if (PosShfOverflow(stmt_string))
        JudgeAndInsert(icond, bop, _rewriter, count, SM,
                       icond->getStmtClassName(), mode, DefHead);
    ...
    if (clang::CompoundStmt *sbody =
            llvm::dyn_cast<clang::CompoundStmt>(ibody)) {
      for (auto &s_stmt : sbody->children()) {
        stmt_string = Tool::get_stmt_string(s_stmt);
        ; // std::cout<<"------check
          // then_body:"<<stmt_string<<"--------"<<std::endl;
      ...
      if (!llvm::isa<clang::IfStmt>(ibody) &&
          !llvm::isa<clang::ForStmt>(ibody) &&
          !llvm::isa<clang::NullStmt>(ibody)) {
        // If ibody is a simple expression/return/decl statement, skip wrapping
        if (llvm::isa<clang::Expr>(ibody) ||
            llvm::isa<clang::ReturnStmt>(ibody) ||
            llvm::isa<clang::DeclStmt>(ibody) ||
            llvm::isa<clang::GotoStmt>(ibody)) {
          if (mode == "null")
            DefHead = ibody->getBeginLoc();
        } else {
          // Atomically replace the statement token-range with a braced block.
          clang::SourceRange SR = get_sourcerange(ibody);
          auto charRange = clang::CharSourceRange::getTokenRange(SR);
          clang::LangOptions LangOpts = _rewriter.getLangOpts();
          llvm::StringRef origRef =
              clang::Lexer::getSourceText(charRange, SM, LangOpts);
      ...
      // const clang::LangOptions& LangOpts = _ctx->getLangOpts();
      // clang::SourceLocation end =
      // clang::Lexer::getLocForEndOfToken(ibody->getEndLoc(), 0, SM, LangOpts);
      // _rewriter.InsertText(end,"}");

      type = ibody->getStmtClassName();

      if (NeedLoopChildren(type))
        LoopChildren(ibody, bop, _rewriter, count, SM,
                     ibody->getStmtClassName(), stmt_string, mode, DefHead);

      else if (mode == "div") {
        if (PosDivideZero(stmt_string))
          JudgeAndInsert(ibody, bop, _rewriter, count, SM,
                         ibody->getStmtClassName(), mode, DefHead);
      } else if (mode == "shf") {
        if (PosShfOverflow(stmt_string))
          JudgeAndInsert(ibody, bop, _rewriter, count, SM,
                         ibody->getStmtClassName(), mode, DefHead);
    ...
    if (ibody)
      if (clang::CompoundStmt *sbody =
        ...
        for (auto &s_stmt : sbody->children()) {
          stmt_string = Tool::get_stmt_string(s_stmt);
          ; // std::cout<<"------check
            // else_body:"<<stmt_string<<"--------"<<std::endl;
          ...
          if (NeedLoopChildren(type))
            LoopChildren(s_stmt, bop, _rewriter, count, SM,
                         s_stmt->getStmtClassName(), stmt_string, mode,
                         DefHead);
        ...
        if (!llvm::isa<clang::IfStmt>(ibody) &&
            !llvm::isa<clang::ForStmt>(ibody) &&
            !llvm::isa<clang::NullStmt>(ibody)) {
          // If ibody is a simple expression/return/decl statement, skip
          // wrapping
          if (llvm::isa<clang::Expr>(ibody) ||
              llvm::isa<clang::ReturnStmt>(ibody) ||
              llvm::isa<clang::DeclStmt>(ibody) ||
    ...
    for (clang::Stmt *subStmt : sbody->children()) {
      ...
      while (clang::CaseStmt *case_stmt =
                 llvm::dyn_cast<clang::CaseStmt>(subStmt)) {
        subStmt = case_stmt->getSubStmt();
      }

      if (clang::DefaultStmt *default_stmt =
              llvm::dyn_cast<clang::DefaultStmt>(subStmt)) {
      ...
      // std::cout<<"=======================switch
      // sub:======================="<<stmt_string<<std::endl;

      if (NeedLoopChildren(type))
        LoopChildren(subStmt, bop, _rewriter, count, SM,
                     subStmt->getStmtClassName(), stmt_string, mode, DefHead);

      else if (mode == "div") {
        if (PosDivideZero(stmt_string))
          JudgeAndInsert(subStmt, bop, _rewriter, count, SM,
                         subStmt->getStmtClassName(), mode, DefHead);
      } else if (mode == "shf") {
        if (PosShfOverflow(stmt_string))
          JudgeAndInsert(subStmt, bop, _rewriter, count, SM,
                         subStmt->getStmtClassName(), mode, DefHead);
...
void find_INTOP(std::pair<int, int> &intopl, std::pair<int, int> &intopr,
                const std::string &fname, int UB_line) {
  std::ifstream file(fname);
  if (!file) {
...
void find_stdlib(const std::string &fname, clang::Rewriter &_rewriter,
                 clang::FileID fid) {
  std::ifstream file(fname);
  if (!file) {
    ; // std::cout<<"file not exists--in find_UB"<<std::endl;
  ...
  if (last_include_line != -1) {
    clang::SourceLocation lineStart =
        SM.translateLineCol(fid, last_include_line, 0);
    insertLoc = clang::Lexer::getLocForEndOfToken(lineStart, 0, SM,
                                                  clang::LangOptions());
  }
...
bool SeqASTVisitor::VisitFunctionDecl(clang::FunctionDecl *func_decl) {
  ...
  if (ContainSpecialChar(func_decl->getNameAsString()))
    return true;
  ...
  if (getFlag(NULL_BIT)) {
    ...
    ; // std::cout<<"\n| Function [" << func_decl->getNameAsString() << "]
      // defined in: " << filePath << "\n";
    int cur_count = count;
    ...
    clang::SourceLocation defHead =
        SM.getExpansionLoc(func_decl->getBeginLoc());
    ...
    *insertCatcher = "";
    ...
    if (func_decl == func_decl->getDefinition()) {
      ...
      ; // std::cout<<"[FuncDecl]current
        // global:"<<global_current_func_name<<std::endl;
      ...
      for (auto &stmt : body_stmt->children()) {
        ...
        clang::SourceLocation insertLoc =
            SM.getExpansionLoc(stmt->getBeginLoc());

        ; // std::cout<<"check:"<<stmt_string<<std::endl;

        if (NeedLoopChildren(type)) {
          LoopChildren(stmt, nullptr, _rewriter, cur_count, SM, type,
                       stmt_string, "null", defHead);
        } else {
    ...
    }; // std::cout<<"check:"<<intopl_loc.first<<":"<<intopl_loc.second<<" &
       // "<<intopr_loc.second<<std::endl;
    ...
    if (func_decl == func_decl->getDefinition()) {
      ...
      // 遍历这个函数中的语句
      // 如果是纯 div 模式（DIV_BIT=1, MUT_BIT=0）且没有 UBFUZZ 标记，
      // 则对函数内所有含 / 或 % 的语句进行无 UBFUZZ 的插桩处理
      if (getFlag(DIV_BIT) && !getFlag(MUT_BIT) && UBFUZZ_line == -1) {
        const clang::BinaryOperator *bop = nullptr;

        for (auto &stmt : body_stmt->children()) {
          auto ori_stmt = stmt;
          clang::SourceLocation insert_loc =
              SM.getExpansionLoc(ori_stmt->getBeginLoc());
          ...
          if (PosDivideZero(stmt_string)) {
            ...
            if (NeedLoopChildren(type))
              ...
              JudgeAndInsert(stmt, bop, _rewriter, count, SM,
                             stmt->getStmtClassName(), "div", defHead);
          } else
            continue;
        }

        clang::SourceLocation InitLoc =
            SM.getExpansionLoc(func_decl->getSourceRange().getBegin());

        if (SM.isWrittenInMainFile(InitLoc)) {
          if (getFlag(DIV_BIT))
            _rewriter.InsertTextBefore(InitLoc,
                                       initSani(last_count, count, SM));
        } else
          ;
        ...
        for (auto &stmt : body_stmt->children()) {
          ...
          if (PosShfOverflow(stmt_string)) {
            if (ori_stmt->getBeginLoc().isMacroID()) {
              ...
              //  auto beginFileLoc =
              //  SM.getSpellingLoc(expansionRange.getBegin()); auto endFileLoc
              //  = SM.getSpellingLoc(expansionRange.getEnd());
              //  clang::LangOptions LangOpts = _ctx->getLangOpts();

              // // 获取源码文本
              // llvm::StringRef macroText = clang::Lexer::getSourceText(
              //     clang::CharSourceRange::getTokenRange(beginFileLoc,
              //     endFileLoc), SM, LangOpts
              // );

              // if (macroText.startswith("do") &&
              // (macroText.trim().endswith("while (0)"))||
              // (macroText.trim().endswith("while(0)"))) {
              //     DO_WHILE_MACRO = true;
              // }
              continue;
            ...
            if (NeedLoopChildren(type))
              ...
              JudgeAndInsert(stmt, bop, _rewriter, count, SM,
                             stmt->getStmtClassName(), "shf", defHead);
        ...
        for (auto &stmt : body_stmt->children()) {
          ...
          if (SM.getSpellingLineNumber(stmt->getEndLoc()) >= UBFUZZ_line &&
              !fir) {
            auto ori_stmt = stmt;
            clang::SourceLocation insert_loc =
                SM.getExpansionLoc(ori_stmt->getBeginLoc());
            ...
            if (const clang::BinaryOperator *bop =
                    llvm::dyn_cast<clang::BinaryOperator>(stmt)) {
              clang::BinaryOperator::Opcode op = bop->getOpcode();
              // 运算符，如op==clang::BinaryOperator::Opcode::BO_Comma
              // 可以判断运算符是不是逗号
              clang::Expr *lhs = bop->getLHS();
              clang::Expr *rhs = bop->getRHS();
              ...
              *insertStr = "";
              ...
              if (getFlag(DIV_BIT)) {
                ...
                if (getFlag(OPT_BIT)) {
                  ...
                  *insertStr = " fprintf(stderr, \"ACT_CHECK_CODE\") &&";
                  if (loc != ori_stmt->getBeginLoc())
                    ...
                    _rewriter.InsertTextBefore(
                        ori_stmt->getBeginLoc(),
                        "fprintf(stderr, \"ACT_CHECK_CODE\");\n");
                } else {
                ...
                _rewriter.InsertTextBefore(
                    SM.getExpansionLoc(ori_stmt->getBeginLoc()), *insertStr);
              }
              delete insertStr;
            }
            //    if(lhs==nullptr || rhs==nullptr) return false;//防止空指针
            //   std::string lhs_class = lhs->getStmtClassName(),rhs_class =
            //   rhs->getStmtClassName();//需要考虑运算符左右子语句可能也是二元运算。所以可能需要设计递归函数
          }
        }
      }
  ...
  }; // std::cout<<"[FunctionDecl]"<<func_decl->getNameAsString()<<"
...
bool SeqASTVisitor::VisitVarDecl(clang::VarDecl *var_decl) {
  ...
  if (var_decl->hasGlobalStorage()) { // && var_decl->isFileVarDecl()
    ...
    // if(type == "BinaryOperator" || type == "CallExpr"){
    //     JudgeAndPtrTrack(expr->getExprStmt(), cur_count, SM, DefHead,
    //     insertLoc);
    // }

    global_w_NameList.insert(var_decl->getNameAsString());
    ; // std::cout<<"[indeed global var
      // inserted]"<<var_decl->getNameAsString()<<std::endl;
...
bool SeqASTVisitor::VisitEnumDecl(clang::EnumDecl *enum_decl) {
  ...
  for (auto *EnumConst : enum_decl->enumerators()) {
    ; // std::cout << "[Enumerator]:" <<
      // EnumConst->getNameAsString()<<std::endl;
    global_w_NameList.insert(EnumConst->getNameAsString());
    ; // std::cout<<"[indeed global var
      // inserted]"<<EnumConst->getNameAsString()<<std::endl;
